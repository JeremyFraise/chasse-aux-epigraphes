...
const objCarte = {
	...
	creerMarqueurs: function () {
		...
		for (let strIdEpigraphe in objJSONepigraphes) {
			...
			if(objEpigrapheCourant.DOMAINE == "Arts visuels") {
				strDomaine = "arts-visuels";
			} 
			else if(objEpigrapheCourant.DOMAINE == "Sciences et Lettres"){
				strDomaine = "sciences-et-lettres";
			}
			else if(objEpigrapheCourant.DOMAINE == "Sports et spectacles"){
				strDomaine = "sports-et-spectacles";
			}
			else{
				strDomaine = "economie-et-politique";
			}
			...
			const strGabaritContenuInfobulle =
				...
				<a class="btn" href="../fiches/${strDomaine}.html?id=${strIdEpigraphe}&titre=${objEpigrapheCourant.PRENOM}-${objEpigrapheCourant.NOM}">Consulter la fiche de ${objEpigrapheCourant.PRENOM} ${objEpigrapheCourant.NOM}</a>
			</div>`;
			...
			// Fiches visitées
			let strVariableBlnFicheVisite = "bln_" + strIdEpigraphe + "_estVisite";
			let blnFicheVisite = JSON.parse(localStorage.getItem(strVariableBlnFicheVisite));
			console.log(blnFicheVisite);
			if(blnFicheVisite == true) {
				objMarqueur.icon = this.strUrlImages + "marqueurs/landmark-navy.svg";
				console.log(objMarqueur.icon);
			}
